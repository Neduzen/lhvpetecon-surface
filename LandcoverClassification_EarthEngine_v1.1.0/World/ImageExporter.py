...
class ImageExporter:
    ...
    def RunImage(self, country, classifier, start_year, end_year):
        ...
        imageProgress = DriveApi.CheckImageProgress(country.GetName(), country.GetExportName(), rasterCells, self.start_year, self.end_year)

        # If not finished, export next cell, else mark as finished
        if len(imageProgress[0]) == 0:
            country.CountryWDB.hasImages = True
            country.CountryWDB.prio = 999  # DELETE LATER???
            country.Save()
            print("no image left")
        else:
    ...
    def RunCellImage(self, cell):
        ...
        for year in range(self.start_year, self.end_year):
            ...
            #export_name = self.country.GetName()
            #export_name = export_name.replace("'", "")
            # inserted this export_name thing due to Cote d'Ivoire
            filename = self.country.GetExportName() + '-image-' + str(cell) + "-" + str(year)
